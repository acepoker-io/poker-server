...
const getUserTransaction = catchAsync(async (req, res,next) => {
  ...
  const transaction = await userService.getUserTransaction(req.user._id, req.query);
  if (!transaction) {
...
const getGameHistory = async (req, res) => {
  ...
  try {
    ...
    const gamesCount = await transactionModel.countDocuments({ userId: userId });
    const lossCount = await transactionModel.countDocuments({ $and: [{ userId: userId }, { amount: { $lt: 0 } }] });
    const winsCount = await transactionModel.countDocuments({ $and: [{ userId: userId }, { amount: { $gt: 0 } }] });
    const populatedFriends = await User.findOne({ _id: userId }).populate({ path: 'friends' });
    // console.log('populatedFriends =============>', populatedFriends);
    const reqFriendsArr = populatedFriends?.friends?.filter((friend) => friend?.status === 3 && !friend?.isBlocked);
    res.status(200).json({ gamesCount, lossCount, winsCount, friendsCount: reqFriendsArr?.length || 0 });
  } catch (err) {
...
const createKyc = catchAsync(async (req, res) => {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    // Check image sizes
    if (IDimageFront[0]?.size > 10 * 1024 * 1024 || IDimageBack[0]?.size > 10 * 1024 * 1024) {
      return res.status(400).send({code:400,message:"Image size should not be greater than 10 MB."});
    }
