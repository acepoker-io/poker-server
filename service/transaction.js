...
const sdk = ThirdwebSDK.fromPrivateKey(process.env.WALLET_PRIVATE_KEY, process.env.CHAIN_ID);
...
export const sendCommisionToSharableAddress = async(amount) => {
    try {
        ...
        const transferToCommisionAddress = await sdk.getProvider().sendTransaction(tx);
